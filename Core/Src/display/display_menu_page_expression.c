...
static uint8_t Display_ExpressionItemIsCcSlot(uint8_t item_index)
{
    return (item_index >= MENU_EXPRESSION_ITEM_CC_FIRST
         && item_index < MENU_EXPRESSION_ITEM_COUNT) ? 1U : 0U;
}

static uint8_t Display_ExpressionItemToSlotIndex(uint8_t item_index)
{
    return (uint8_t)(item_index - MENU_EXPRESSION_ITEM_CC_FIRST);
}
...
static uint16_t Display_GetExpressionHeelRawValue(const RuntimeConfigGlobal_t *global)
{
    if (!global)
        return 0U;

    return global->expression_pedal_invert
        ? global->expression_pedal_max_raw
        : global->expression_pedal_min_raw;
}

static uint16_t Display_GetExpressionToeRawValue(const RuntimeConfigGlobal_t *global)
{
    if (!global)
        return 0U;
                                                   ...
                                                   const RuntimeConfigExpressionPedalCcSlot_t *slot)
{
    ...
    value_x = Display_GetMenuRightAlignedExpressionValueX("CC:--- Heel:--- Toe:---");
    ...
    value_x = Display_MenuRowComposeValueSegment32(value_x, " Heel:", 0U);
    value_x = Display_MenuRowComposeValueSegment32(value_x,
                                                   heel_text,
                                                   (display_state.menu_expression_field_index == 1U) ? 1U : 0U);
    value_x = Display_MenuRowComposeValueSegment32(value_x, " Toe:", 0U);
...
void Display_FormatExpressionValue(uint8_t item_index, char *buffer, size_t buffer_size)
{
    ...
    if (item_index == MENU_EXPRESSION_ITEM_MODE)
    {
    ...
    if (item_index == MENU_EXPRESSION_ITEM_SET_HEEL)
    {
        (void)snprintf(buffer, buffer_size, "%4u", Display_GetExpressionHeelRawValue(global));
        return;
    }

    if (item_index == MENU_EXPRESSION_ITEM_SET_TOE)
    {
        (void)snprintf(buffer, buffer_size, "%4u", Display_GetExpressionToeRawValue(global));
        return;
    }

    if (item_index == MENU_EXPRESSION_ITEM_INVERT)
    {
        (void)snprintf(buffer, buffer_size, "%s", global->expression_pedal_invert ? "Yes" : "No");
        return;
    }

    if (!Display_ExpressionItemIsCcSlot(item_index))
    {
        buffer[0] = '\0';
        return;
    }
    ...
    {
        const RuntimeConfigExpressionPedalCcSlot_t *slot =
            &global->expression_pedal_cc_slots[Display_ExpressionItemToSlotIndex(item_index)];

        if (slot->cc == PRESET_CC_NUMBER_UNUSED)
            (void)snprintf(buffer, buffer_size, "CC:--- Heel:%3u Toe:%3u", slot->heel_value, slot->toe_value);
        else
            (void)snprintf(buffer, buffer_size, "CC:%3u Heel:%3u Toe:%3u", slot->cc, slot->heel_value, slot->toe_value);
    }
}
...
void Display_DrawMenuExpressionItem(uint8_t item_index)
{
    ...
    if (Display_ExpressionItemIsCcSlot(item_index) && row_selected)
    {
        if (global)
            Display_DrawExpressionCcEditRowByIndex(row_index,
                                                   Display_ExpressionItemToSlotIndex(item_index),
                                                   &global->expression_pedal_cc_slots[
                                                       Display_ExpressionItemToSlotIndex(item_index)]);
        else
            Display_ClearStandardMenuRow(row_index);
    ...
    else if (item_index == MENU_EXPRESSION_ITEM_SET_HEEL)
        (void)snprintf(label_text, sizeof(label_text), "Set Heel");
    else if (item_index == MENU_EXPRESSION_ITEM_SET_TOE)
        (void)snprintf(label_text, sizeof(label_text), "Set Toe");
    else if (item_index == MENU_EXPRESSION_ITEM_INVERT)
        (void)snprintf(label_text, sizeof(label_text), "Invert");
    else
        Display_FormatExpressionSlotLabel(Display_ExpressionItemToSlotIndex(item_index), label_text, sizeof(label_text));
